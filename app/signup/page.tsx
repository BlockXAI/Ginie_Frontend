...
export default function SignUpPage() {
  ...
  const friendlyAuthError = (err: any, mode: "signup" | "signin") => {
    const status = err?.status as number | undefined;
    const code = String(err?.code || err?.message || "").toLowerCase();
    if (mode === "signup" && (status === 409 || code.includes("user_exists") || code.includes("already") || code.includes("exists"))) {
      return "User already exists. Please sign in or use a different email.";
    }
    if (mode === "signin" && (status === 404 || code.includes("user_not_found") || (code.includes("not") && code.includes("found")))) {
      return "User not found. Please create an account first or sign up.";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (step === "send") {
        const res = await api.sendOtp(email.trim(), name.trim(), "signup");
        if ((res as any)?.ok === false) {
          setError(friendlyAuthError({ code: (res as any)?.error?.code }, "signup"));
          return;
        }
        ...
        await api.verifyOtp(email.trim(), otp.trim(), challengeId, { mode: "signup", name: name.trim() });
        setAuthCookie();
        // Optionally hydrate user before redirect
      ...
      >
        <div className="relative z-10 w-full max-w-md px-4 flex flex-col items-center justify-center gap-6">
          <div ref={headingWrapRef}>
            <AuthHeading title="Sign up" className="mb-0" />
          </div>
