...
import { Loader2, Trash2, Download, ExternalLink, ChevronDown, ChevronUp, Globe, FileCode, CheckCircle2, XCircle, Clock, Play, Copy, Check, Shield, ShieldCheck } from "lucide-react";
          ...
          {builderProjects.length === 0 ? (
            ...
            <div className="space-y-2">
              {builderProjects.map((p: any) => (
                <div key={p.id} className="flex items-center justify-between gap-3 p-3 rounded bg-white/[0.02] border border-white/5">
                  <div className="min-w-0">
                    <div className="text-white/90 font-medium truncate">
                      {p.title || `Project ${String(p.id).slice(0, 8)}`}
                    </div>
                    <div className="text-xs text-white/50 mt-0.5">
                      {p.status || "unknown"}
                      {p.vercel_url ? ` • ${p.vercel_url}` : ""}
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={() => router.push(`/chat/${encodeURIComponent(p.id)}?mode=builder`)}
                    className="text-xs px-3 py-1 rounded bg-white/5 hover:bg-white/10 text-white/80 shrink-0"
                  >
                    Open
                  </button>
                </div>
